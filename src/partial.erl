...
partial_application_arity(List) ->
   case 
      lists:filter(fun(X) -> X =/= undefined end, lists:map(fun arity/1, List))
   of
      [] -> undefined;
      N  -> lists:max(N)
   end.
...
transform_partial_application(N, Line, F0, As0) ->
   As1 = partial_application_args(N, As0),
   F1  = partial_application_lambda(Line, F0, As1),
   partial_application_call(2, N, Line, F1).

partial_application_args(0, As0) ->
   As0;
partial_application_args(N, As0) ->
   As1 = lists:map(fun(X) -> args(N, X) end, As0),
   partial_application_args(N - 1, As1).

args(N, {var, Line, X} = Var) ->
   L = integer_to_list(N),
   case atom_to_list(X) of
      [$_] when N == 1 -> 
         {var, Line, list_to_atom("_Vpa" ++ L)};
      [$_|L] -> 
         {var, Line, list_to_atom("_Vpa" ++ L)};
      _     ->
         Var
...
partial_application_lambda(Line, F0, As0) ->
   {'fun', Line,
      {clauses, [
         {clause, Line,
            [{var, Line, '_Vpa1'}],
...
partial_application_call(I, N, Line, F0)
 when I =< N ->
   F1 = {'fun', Line,
      {clauses, [
         {clause, Line,
            [{var, Line, list_to_atom("_Vpa" ++ integer_to_list(I))}],
            [],
            [F0]
         }
      ]}
   },
   partial_application_call(I + 1, N, Line, F1);

partial_application_call(_, _, _, F0) ->
   F0.
