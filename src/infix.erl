...
hook_infix({op, Line, '/', {op, _, '/', {var, _, '_'}, {remote, _, _, _} = Op}, R0}) ->
    lpartial(Op, Line, uuid(), expr(R0));

hook_infix({op, Line, '/', {op, _, '/', L0, {remote, _, _, _} = Op}, {var, _, '_'}}) ->
    rpartial(Op, Line, expr(L0), uuid());

hook_infix({op, Line, '/', {op, _, '/', L0, {remote, _, _, _} = Op}, R0}) ->
    L1 = expr(L0),
    R1 = expr(R0),
    {call, Line, Op, [L1, R1]};


hook_infix({op, Line, Op, L0, R0}) ->
    L1 = expr(L0),
    R1 = expr(R0),
    {op,Line,Op,L1,R1}.
...
%%
%%
lpartial(Op, Line, Var, R) ->
    {'fun', Line,
        {clauses, [
            {clause, Line,
                [{var, Line, Var}],
                [],
                [{call, Line, Op, [{var, Line, Var}, R]}]
            }
        ]}
    }.
...
%%
%%
rpartial(Op, Line, L, Var) ->
    {'fun', Line,
        {clauses, [
            {clause, Line,
                [{var, Line, Var}],
                [],
                [{call, Line, Op, [L, {var, Line, Var}]}]
            }
        ]}
    }.
...
%% unique variable
uuid() ->
   list_to_atom("_Vpi" ++ integer_to_list(unique())).
